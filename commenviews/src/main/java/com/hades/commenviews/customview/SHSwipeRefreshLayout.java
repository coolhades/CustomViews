...
public class SHSwipeRefreshLayout extends FrameLayout implements NestedScrollingParent {
    ...
    private static final int GUIDANCE_VIEW_HEIGHT = 60;
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        int resId;
        TypedArray ta = context.obtainStyledAttributes(attrs, com.scu.miomin.shswiperefresh.R.styleable.SHSwipeRefreshLayout);
        Resources resources = context.getResources();

        //Indicator背景颜色
        resId = ta.getResourceId(com.scu.miomin.shswiperefresh.R.styleable.SHSwipeRefreshLayout_guidance_view_bg_color, -1);
        if (resId == -1) {
            mGuidanceViewBgColor = ta.getColor(com.scu.miomin.shswiperefresh.R.styleable.SHSwipeRefreshLayout_guidance_view_bg_color,
                    Color.WHITE);
        } else {
        ...
        //加载文字颜色
        resId = ta.getResourceId(com.scu.miomin.shswiperefresh.R.styleable.SHSwipeRefreshLayout_guidance_text_color, -1);
        if (resId == -1) {
            mGuidanceViewTextColor = ta.getColor(com.scu.miomin.shswiperefresh.R.styleable.SHSwipeRefreshLayout_guidance_text_color,
                    Color.BLACK);
        } else {
        ...
        //进度条背景颜色
        resId = ta.getResourceId(com.scu.miomin.shswiperefresh.R.styleable.SHSwipeRefreshLayout_progress_bg_color, -1);
        if (resId == -1) {
            mProgressBgColor = ta.getColor(com.scu.miomin.shswiperefresh.R.styleable.SHSwipeRefreshLayout_progress_bg_color,
                    Color.WHITE);
        } else {
        ...
        //进度条颜色
        resId = ta.getResourceId(com.scu.miomin.shswiperefresh.R.styleable.SHSwipeRefreshLayout_progress_bar_color, -1);
        if (resId == -1) {
            mProgressColor = ta.getColor(com.scu.miomin.shswiperefresh.R.styleable.SHSwipeRefreshLayout_progress_bar_color,
                    Color.RED);
        } else {
        ...
        //下拉刷新文字描述
        resId = ta.getResourceId(com.scu.miomin.shswiperefresh.R.styleable.SHSwipeRefreshLayout_refresh_text, -1);
        if (resId == -1) {
            mRefreshDefaulText = ta.getString(com.scu.miomin.shswiperefresh.R.styleable.SHSwipeRefreshLayout_refresh_text);
        } else {
            mRefreshDefaulText = resources.getString(resId);
        }

        //上拉加载文字描述
        resId = ta.getResourceId(com.scu.miomin.shswiperefresh.R.styleable.SHSwipeRefreshLayout_load_text, -1);
        if (resId == -1) {
            mLoadDefaulText = ta.getString(com.scu.miomin.shswiperefresh.R.styleable.SHSwipeRefreshLayout_load_text);
        } else {
    ...
    private boolean moveGuidanceView(float distanceY) {
        ...
        if (!canChildScrollUp() && mPullRefreshEnable && mCurrentAction == ACTION_PULL_REFRESH) {
            ...
            lp.height -= distanceY/3.5f; //阻尼效果 待优化
            if (lp.height < 0) {
                lp.height = 0;
            }
